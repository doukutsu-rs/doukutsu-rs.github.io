...
export function ScreenshotGallery({ screenshots }: ScreenshotGalleryProps) {
  ...
  const selectedOptionTrigger = useRef<HTMLButtonElement | null>(null);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
        {screenshots.map((screenshot, index) => (
          <DialogTrigger
            key={screenshot.src}
            className="relative aspect-video overflow-hidden rounded-lg border border-gray-800 cursor-pointer group transition-shadow shadow-md hover:shadow-lg hover:shadow-orange-900 hover:border-orange-700 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-black"
            onClick={(e) => {
              openGallery(index);
              selectedOptionTrigger.current = e.currentTarget;
            }}
            aria-label={`View image: ${screenshot.alt}`}
      ...
      <DialogPortal data-slot="dialog-portal">
        ...
        <DialogPrimitive.Content
          ...
          onCloseAutoFocus={(event) => {
            if (selectedOptionTrigger.current) {
              event.preventDefault();
              selectedOptionTrigger.current.focus();
            }
          }}
        >
